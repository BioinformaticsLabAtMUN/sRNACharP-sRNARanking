...
if (params.help) {
  ...
  log.info '--classifier  CLASSIFIER_FOR_sRNA the random forest classifier file [required]'
  exit 1
}
...
rf_Classifier_File = file("${params.dir}/${params.classifier}")
...
process sRNA_Ranking{

    input:
    file "FeatureTable.tsv" from featureTableInput
    file "RF_classifier4sRNA.sav" from rf_Classifier_File
    output:
    file "rankedProbability.txt" into rankedProbability
    ...
    data = pandas.read_table("FeatureTable.tsv", index_col=0)
    ...
    output_data_table.to_csv("rankedProbability.txt",sep="\\t")
    """
}

rankedProbability.collectFile(name: file("rankedProbability.txt"))
/*************************************************************************************
* END of Workflow
*************************************************************************************/
workflow.onComplete { 
